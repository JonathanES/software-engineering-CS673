...
let milestoneID = 1;
const milestoneName = 'The Rolling Milestones';
const nmilestoneName = 'Too stoned to mile';
    ...
    client.on('connect', function () {
      ...
      client.on('CREATE_MILESTONE', data => {
        ...
        client.on('GET_MILESTONE', ress => {
          expect(ress[0].MilestoneName).to.equal('The Rolling Milestones');
  ...
  it('should rename the milestone', function(done) {
    ...
    client.on('connect', function () {
        ...
        client.on('UPDATE_MILESTONE_NAME', data => {
            //console.log(data);
            expect(data).to.equal('Too stoned to mile');
            client.disconnect();
            done();
        });
    });
